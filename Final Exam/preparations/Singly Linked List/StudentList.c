...
{	
    ...
    { 
        l->Last->Next = e; // Link the current last entry to the new entry
...
{	
    ...
    { 
        ...
        if (!l->First) // If the list is now empty after removal
        { 
...
void InsertStudentAfter(SENTRY *e, STUDENT *s)
{
    ...
    assert(e);   // Ensure the input entry is not NULL
    assert(s);   // Ensure the input student is not NULL
    ...
    if (e->List->Last == e) // If the existing entry is the last in the list
    { 
        e->List->Last = New; // Update the list's `Last` pointer to the new entry
    }
